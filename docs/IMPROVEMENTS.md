...
## Recently Implemented (Gemini 3.1 Flash TTS Upgrade)

The following items from this list were shipped as part of the Gemini 3.1 Flash TTS upgrade:

| # | Feature | Details |
|---|---------|--------|
| F2 | **Multi-speaker TTS** | Dialogue mode in Script Reader with dual voice selectors, `POST /api/voices/tts/multi` endpoint, Web Audio playback & WAV download. |
| F11 | **Audio tags (emotion / style)** | 22 audio tags across 3 categories (style, emotion, action) via `AudioTagsToolbar` with cursor-position insertion in Script Reader. |
| F14 | **Multi-language TTS** | 36-language selector (70+ supported) in `AiTtsPreview` with `languageCode` threaded through Go backend to Gemini API. |
| F26 | **Retry logic for TTS 500 errors** | 3-retry with exponential backoff on HTTP 500 in `GenerateTTS` and `GenerateMultiSpeakerTTS`. |
| Q1 | **Retry logic** | Same as F26. |
| Q8 | **Audio tag insertion buttons** | Same as F11. |
...
| F2 | ~~**Multi-speaker TTS**~~ ✅ | ~~Leverage Gemini's `MultiSpeakerVoiceConfig` to cast different voices for different characters in a single script.~~ **Implemented** — Dialogue mode in Script Reader with dual voice selectors and `POST /api/voices/tts/multi`. |
...
| F11 | ~~**Emotion / style tags**~~ ✅ | ~~Expose audio tags as clickable buttons that insert into the script.~~ **Implemented** — `AudioTagsToolbar` with 22 tags in 3 categories (style, emotion, action) with cursor-position insertion. |
...
| F14 | **Language detection & routing** ⚡ | ~~Auto-detect script language and suggest the best voice + `languageCode`.~~ **Partially implemented** — 36-language selector added to `AiTtsPreview`. Auto-detection remains a future enhancement. |
...
| # | Feature | Description |
|---|---------|-------------|
| F26 | ~~**Retry logic for TTS 500 errors**~~ ✅ | ~~Add automatic retry with exponential backoff.~~ **Implemented** — 3-retry with exponential backoff (1s, 2s, 4s) on HTTP 500 in both `GenerateTTS` and `GenerateMultiSpeakerTTS`. |
...
| F32 | **Model selection** | Default model upgraded to `gemini-3.1-flash-tts-preview`. A UI toggle for choosing between available TTS models per generation remains a future enhancement. |
...
| Q10 | Model selection toggle (default now 3.1 Flash TTS; UI toggle TBD) | Backend + UI |

---

*Last updated: June 2025 — Gemini 3.1 Flash TTS upgrade*
